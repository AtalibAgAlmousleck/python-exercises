...
class Student: 
    ...
    def save_course_to_file(course_id: str, course_name: str, instructor: str):
        # try an catch the error if present before saving details.
        try:
            ...
            print(f"Ops! failed to write to file: {e}")

    @staticmethod #@classmethod get_student supose to be static method not classmethod
    def get_student() -> 'Student':
        while True:
            student_name = input("Enter your full name: ").strip()
            #if not cls.is_valid_name(student_name):
            if not Student.is_valid_name(student_name):
                print("Name is invalid!")
                continue
            
            student_email = input("Enter email: ").strip()
            #if not cls.is_valid_email(student_email):
            if not Student.is_valid_email(student_email):
            ...
            if not Student.is_valid_gender(student_gender):
    ...
    def is_valid_email(email: str) -> bool:
        domains_pattern = "|".join(VALID_EMAIL_DOMAINS)
        return bool(re.fullmatch(rf"^[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.({domains_pattern})$", email, re.IGNORECASE))
